...
out_dir = f"../RTDL/output/catboost_configs_tuned"
...
for model in models:
    for dset_id in dset_ids:
        ...
        for seed in range(seeds):
            ...
            # launch_str = f"python -W ignore bin/{model}.py output/catboost_configs_tuned/{dset_id}_supervised_pretraining_ft-trans_seed{seed}.toml -f \n" \
            #              f"python -W ignore bin/{model}.py output/catboost_configs_tuned/{dset_id}_mlm_pretraining_ft-trans_seed{seed}.toml -f \n"
            # launch_str = f"python -W ignore bin/{model}.py output/catboost_configs_tuned/{dset_id}_supervised_pretraining_{model}_seed{seed}.toml -f \n"
            ...
            for sample_num in sample_nums:
                for n_est in n_estimators:
                    ...
                    # with open(os.path.join(out_dir,
                    #                        f"{dset_id}_downstream_{sample_num}samples_linear_head_tuned_full_from_supervised_pretrain_{model}_seed{seed}.toml"),
                    #           "w") as fh:
                    #     toml.dump(toml_dict, fh)
                    #
                    ...
                    # toml_dict["transfer"]["use_mlp_head"] = True
                    # with open(os.path.join(out_dir,
                    #                        f"{dset_id}_downstream_{sample_num}samples_mlp_head_tuned_full_from_supervised_pretrain_{model}_seed{seed}.toml"),
                    #           "w") as fh:
                    #     toml.dump(toml_dict, fh)
                    ...
                    launch_str += f"python -W ignore bin/{model}_.py output/catboost_configs_tuned/{dset_id}_downstream_{sample_num}samples_fromscratch_{model}_nestimators{n_est}_seed{seed}.toml -f \n"

            os.makedirs('../RTDL/launch/tuned_catboost_jobs', exist_ok=True)
            with open(f"../RTDL/launch/tuned_catboost_jobs/{dset_id}_{model}_exp_seed{seed}.sh", "w") as fh:
                fh.write(launch_str)