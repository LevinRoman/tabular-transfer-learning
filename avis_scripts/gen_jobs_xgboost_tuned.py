...
out_dir = f"../RTDL/output/xgboost_configs_tuned"
...
for model in models:
    for dset_id in dset_ids:
        ...
        for seed in range(seeds):
            ...
            # launch_str = f"python -W ignore bin/{model}.py output/xgboost_configs_tuned/{dset_id}_supervised_pretraining_ft-trans_seed{seed}.toml -f \n" \
            #              f"python -W ignore bin/{model}.py output/xgboost_configs_tuned/{dset_id}_mlm_pretraining_ft-trans_seed{seed}.toml -f \n"
            # launch_str = f"python -W ignore bin/{model}.py output/xgboost_configs_tuned/{dset_id}_supervised_pretraining_{model}_seed{seed}.toml -f \n"
            ...
            for sample_num in sample_nums:
                for n_est in n_estimators:
                    ...
                    launch_str += f"python -W ignore bin/{model}_.py output/xgboost_configs_tuned/{dset_id}_downstream_{sample_num}samples_fromscratch_{model}_nestimators{n_est}_seed{seed}.toml -f \n"

            os.makedirs('../RTDL/launch/tuned_xgboost_jobs', exist_ok=True)
            with open(f"../RTDL/launch/tuned_xgboost_jobs/{dset_id}_{model}_exp_seed{seed}.sh", "w") as fh:
                fh.write(launch_str)