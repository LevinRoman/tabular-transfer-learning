...
# Defining parameters
seeds = 5
models = ['resnet', 'mlp', 'saint'] #ft_transformer_mlm for Avi's things
dset_ids = [188, 541, 1483, 40687, 40975, 41169, 42726, 42728, "california_housing", "jannis"]
out_dir = f"../RTDL/output/deep_configs"
...
for model in models:
    for dset_id in dset_ids:
        ...
        for seed in range(seeds):
            ...
            launch_str = f"python -W ignore bin/{model}.py output/deep_configs/{dset_id}_supervised_pretraining_{model}_seed{seed}.toml -f \n"
            ...
            for sample_num in sample_nums:
                ...
                with open(os.path.join(out_dir,
                                       f"{dset_id}_downstream_{sample_num}samples_fromscratch_{model}_seed{seed}.toml"),
                          "w") as fh:
                    toml.dump(toml_dict, fh)
