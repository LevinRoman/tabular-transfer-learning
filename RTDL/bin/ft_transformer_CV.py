...
if __name__ == "__main__":
    ...
    zero.set_randomness(args['seed'])
    #Never do CV on upstream data!!!
    assert 'downstream' in args['transfer']['stage']
    if args['transfer']['downstream_samples_per_class'] == 2:
        cv_outer = LeaveOneOut()
    else:
        cv_outer = StratifiedKFold(n_splits=5, shuffle=True, random_state=args['seed'])
    cv_train_score = []
    cv_val_score = []
    cv_test_score = []
    C_full = copy.deepcopy(C)
    N_full = copy.deepcopy(N)
    y_full = copy.deepcopy(y)
    for train_idx, val_idx in tqdm.tqdm(cv_outer.split(y_full[lib.TRAIN], y_full[lib.VAL])):
        # split data
        if C_full is not None:
            C[lib.TRAIN], C[lib.VAL] = C_full[lib.TRAIN][train_idx], C_full[lib.TRAIN][val_idx]
        if N_full is not None:
            N[lib.TRAIN], N[lib.VAL] = N_full[lib.TRAIN][train_idx], N_full[lib.TRAIN][val_idx]
        y[lib.TRAIN], y[lib.VAL] = y_full[lib.TRAIN][train_idx], y_full[lib.TRAIN][val_idx]
        ...
        cv_train_score.append(metrics['train']['score'])
        cv_val_score.append(metrics['val']['score'])
        cv_test_score.append(metrics['test']['score'])
        # %%
    print('\nRunning the final evaluation...')
    stats['metrics'] = {'train':
                            {'score': np.mean(cv_train_score)},
                        'val':
                            {'score': np.mean(cv_val_score)},
                        'test':
                            {'score': np.mean(cv_test_score)}}
