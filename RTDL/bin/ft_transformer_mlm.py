...
class sep_MLP(nn.Module):
    def __init__(self, dim, len_feats, categories):
        ...
        for i in range(len_feats):
            self.layers.append(simple_MLP([dim, dim * 5, categories[i]]))
...
class Transformer(nn.Module):
    ...
    def __init__(
        ...
        if categories:
            self.d_categorical = len(categories)
        else:
            self.d_categorical = 0
        ...
        mlm_mask_offset = F.pad(torch.Tensor(self.d_numerical + self.d_categorical + 1).fill_(2).type(torch.int8),
                                (1, 0), value=0)
        mlm_mask_offset = mlm_mask_offset.cumsum(dim=-1)[:-1]
        self.register_buffer('mlm_mask_offset', mlm_mask_offset)
        self.mlm_mask_embeds = nn.Embedding((self.d_numerical + self.d_categorical + 1) * 2, d_token)
        self.mlp1 = sep_MLP(d_token, self.d_categorical, categories)
        self.mlp2 = sep_MLP(d_token, self.d_numerical, np.ones(self.d_numerical).astype(int))
    ...
    def transform(self, x):
        ...
        x_cont = x[:, :self.d_numerical + 1]
        x_cat = x[:, self.d_numerical + 1:]

        return x, x_cat, x_cont

    def mlm_embed_token_mask(self, x_enc, mlm_mask):
        mlm_mask_temp = mlm_mask + self.mlm_mask_offset.type_as(mlm_mask)
        mlm_mask_temp = self.mlm_mask_embeds(mlm_mask_temp)
        x_enc[mlm_mask == 0] = mlm_mask_temp[mlm_mask == 0]
        return x_enc
...
# def mlm_ssl(model, x_num, x_cat, loss_masked_only=False, mode='mlm'):
#     """Mode can be mlm or random_token_replacement
#     We should probably combine the two also
#     """
#     criterion_classification = nn.CrossEntropyLoss()
#     criterion_regression = nn.MSELoss(reduction="none")
#     x_enc = model.tokenizer(x_num, x_cat)
#     ic(x_cat)
#     ic(x_num)
#     ic(loss_masked_only)
#     ic(mode)
#     # generate mlm masks
#     # shape (bs, num_features) True if we don't mask
#     if x_cat is not None:
#         sample = torch.rand((x_cat.shape[0], x_cat.shape[1])).topk(1, dim=1).indices
#         mlm_cat_mask = torch.ones((x_cat.shape[0], x_cat.shape[1]))
#         mlm_cat_mask.scatter_(dim=1, index=sample, value=0)
#         mlm_cat_mask = mlm_cat_mask.long().to(device)
#     else:
#         mlm_cat_mask = torch.rand((x_num.shape[0], 0)).long().to(device)
#
#     # mask for numerical features without [cls]
#     if x_num is not None:
#         sample = torch.rand((x_num.shape[0], x_num.shape[1])).topk(1, dim=1).indices
#         mlm_num_mask = torch.ones((x_num.shape[0], x_num.shape[1]))
#         mlm_num_mask.scatter_(dim=1, index=sample, value=0)
#         mlm_num_mask = mlm_num_mask.long().to(device)
#     else:
#         mlm_num_mask = torch.rand((x_cat.shape[0], 0)).long().to(device)
...
#     if "mlm" in mode:
#         ic()
#         # adding cls token mask
#         mlm_mask = torch.cat([torch.ones(mlm_num_mask.shape[0], 1).bool().to(device),
#                               mlm_num_mask, mlm_cat_mask],
#                              dim=1)
#         ic(mlm_mask)
#         x_enc_masked = model.mlm_embed_token_mask(x_enc, mlm_mask)
#     elif mode == 'random_token_replacement':
#         # permute categorical features (independently in each column, over batch)
#         # and replace 100*p% of original features with permuted:
#         random_categ = torch.zeros_like(x_cat)
#         x_categ_replaced = x_cat.clone()
#         for feat_idx in range(x_cat.shape[1]):  # don't permute cls, but it always has the same value though
#             random_categ[:, feat_idx] = x_cat[:, feat_idx][torch.randperm(x_cat.shape[0], device=device)]
#         x_categ_replaced[~mlm_cat_mask.bool()] = random_categ[~mlm_cat_mask.bool()]
#
#         # permute numerical features and replace 100*p% of original features with permuted
#         random_cont = torch.zeros_like(x_num)
#         x_cont_replaced = x_num.clone()
#         for feat_idx in range(x_num.shape[1]):
#             random_cont[:, feat_idx] = x_num[:, feat_idx][torch.randperm(x_num.shape[0], device=device)]
#         x_cont_replaced[~mlm_num_mask.bool()] = random_cont[~mlm_num_mask.bool()]
#
#         # recompute embeddings, preserve nans
#         x_enc_masked = model.tokenizer(x_cont_replaced, x_categ_replaced)
#     else:
#         raise NotImplementedError('Mode should either be mlm or random_token_replacement')
#
#     # get features of masked data
#     x, _, _ = model.transform(x_enc_masked)
#     # predict inputs
#     cat_outs = model.mlp1(x[:, model.d_numerical + 1:, :])
#     num_outs = model.mlp2(x[:, 1:model.d_numerical + 1, :])
#     if len(num_outs) > 0:
#         if loss_masked_only:
#             l2 = 0
#             for con_idx in range(len(num_outs)):
#                 con_samples_unmasked = mlm_num_mask[:, con_idx].bool()
#                 l2_temp = criterion_regression(num_outs[con_idx][~con_samples_unmasked][:, 0],
#                                            x_num[:, con_idx][~con_samples_unmasked])
#                 if not torch.isnan(l2_temp.mean()):
#                     l2 += l2_temp.mean()
#                 else:
#                     ic(con_samples_unmasked)
#                     ic(num_outs[con_idx][~con_samples_unmasked][:, 0])
#                     ic(x_num[:, con_idx][~con_samples_unmasked])
#                     ic(l2_temp)
#                     ic(con_idx)
#                     exit()
#                 # ic(con_idx, l2)
#         else:
#             num_outs = torch.cat(num_outs, dim=1)
#             l2 = criterion_regression(num_outs, x_num)
#     else:
#         l2 = 0
#     l1 = 0
...
#     #     if loss_masked_only:
#     #         cat_samples_unmasked = mlm_cat_mask[:, j].bool()
#     #         l1 += criterion_classification(cat_outs[j][~cat_samples_unmasked], x_cat[:, j][~cat_samples_unmasked])
#     #     else:
#     #         l1 += criterion_classification(cat_outs[j], x_cat[:, j])
#     loss = l1 + l2
#
#     # print(x_num[0])
#     # print(num_outs[0])
...
if __name__ == "__main__":
    ...
    if "self_supervised" in args["training"].keys():
        self_supervised = args['training']['self_supervised']
    else:
        self_supervised = False
    if self_supervised:
        sf_loss_lam = args['training']['sf_loss_lam']
    ...
    model = Transformer(
        ...
        categories=lib.get_categories_full_cat_data(full_cat_data_for_encoder),
        # I think it's # of unique values in each cat variable
        d_out=D.info['n_classes'] if D.is_multiclass or D.is_binclass else 1,
        # multilabel in case of pretraining binclass
        **args['model'],
    ).to(device)
    ...
    for epoch in stream.epochs(args['training']['n_epochs']):
        ...
        for batch_idx in epoch:
            ...
            if self_supervised:
                ssl_loss = mlm_ssl(model, X_num_batch, X_cat_batch,
                                   loss_masked_only=args['training']['mlm_loss_masked_only'],
                                   mode=args['training']['mlm_mode'])
                loss = ssl_loss
                # print(f"loss: {loss}")
                if "supervised" in args['training']['mlm_mode']:
                    model_output = model(X_num_batch, X_cat_batch)
                    loss += args["training"]["sf_loss_lam"] * loss_fn(model_output, Y_device['train'][batch_idx])
            else:
                model_output = model(X_num_batch, X_cat_batch)
