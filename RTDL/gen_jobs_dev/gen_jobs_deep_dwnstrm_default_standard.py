...
out_dir = f"output_mimic_dev/deep_dwnstrm_default_standard"
    ...
    for downstream_target in downstream_targets:
        for dset_id in dset_ids:
            ...
            for seed in range(seeds):
                ...
                os.makedirs('launch_mimic_dev/deep_dwnstrm_default_standard', exist_ok=True)
                with open(f"launch_mimic_dev/deep_dwnstrm_default_standard/{dset_id}{downstream_target}_{model}_exp_seed{seed}.sh", "w") as fh:
                    fh.write(launch_str)
                global_launch_str += f"python3 dispatch_single_node.py launch_mimic_dev/deep_dwnstrm_default_standard/{dset_id}{downstream_target}_{model}_exp_seed{seed}.sh --name {dset_id}{downstream_target}_{model}_exp_seed{seed} --time {time} \n"

os.makedirs('launch_mimic_dev/global_launch', exist_ok=True)
with open(f"launch_mimic_dev/global_launch/deep_dwnstrm_default_standard.sh", "w") as fh:
