...
def main():
    ...
    os.makedirs('results_dev/model_tables', exist_ok=True)
    if not os.path.exists('results_dev/model_tables/all_results.csv') or args.force:
        df = get_exp_results(args.filepath)
        df.to_csv('results_dev/model_tables/all_results.csv', index = False)
    else:
        df = pd.read_csv('results_dev/model_tables/all_results.csv')
    print(df.head())
    df_full = df.copy()

    if not os.path.exists('results_dev/model_tables/all_results_seed_averaged_epoch_max.csv') or args.force:
        # average over seeds
        ...
        table.to_csv('results_dev/model_tables/all_results_seed_averaged_epoch_max.csv', index = False)
    else:
        table = pd.read_csv('results_dev/model_tables/all_results_seed_averaged_epoch_max.csv')
    print(table.head(100))

    models_to_build = ['ft_transformer', 'resnet']
    for cur_model in models_to_build:
        ...
        os.makedirs('results_dev/model_tables/{}'.format(cur_model), exist_ok = True)
        df_score.to_csv('results_dev/model_tables/{}/df_score.csv'.format(cur_model), float_format = '%.4f')
        df_sem.to_csv('results_dev/model_tables/{}/df_sem.csv'.format(cur_model), float_format = ".4f")
