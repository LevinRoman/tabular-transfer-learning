...
out_dir = f"../RTDL/output/avi"
sample_nums = [2, 5, 10, 50, 100]
os.makedirs(out_dir, exist_ok=True)


for dset_id in dset_ids:
    optuna_config_path = f"../RTDL/optuna_configs_pretrained/{dset_id}/ft_transformer/0/best.toml"
    ...
    for seed in range(seeds):
        ...
        with open(os.path.join(out_dir, f"{dset_id}_supervised_pretraining_ft-trans_seed{seed}.toml"), "w") as fh:
        ...
        launch_str = f"python -W ignore bin/ft_transformer_mlm.py output/avi/{dset_id}_supervised_pretraining_ft-trans_seed{seed}.toml -f \n" \
                     f"python -W ignore bin/ft_transformer_mlm.py output/avi/{dset_id}_mlm_pretraining_ft-trans_seed{seed}.toml -f \n"
        ...
        for sample_num in sample_nums:
            ...
            with open(os.path.join(out_dir,
                                   f"{dset_id}_downstream_{sample_num}samples_fromscratch_ft-trans_seed{seed}.toml"),
                      "w") as fh:
                toml.dump(toml_dict, fh)
            ...
            with open(os.path.join(out_dir,
                                   f"{dset_id}_downstream_{sample_num}samples_random_extractor_ft-trans_seed{seed}.toml"),
                      "w") as fh:
                toml.dump(toml_dict, fh)
            ...
            # toml_dict["transfer"]["downstream_samples_per_class"] = sample_num
            # with open(os.path.join(out_dir,
            #                        f"{dset_id}_downstream_{sample_num}samples_tuned_linear_head_from_supervised_pretrain_ft-trans_seed{seed}.toml"),
            #           "w") as fh:
            #     toml.dump(toml_dict, fh)
            ...
            with open(os.path.join(out_dir,
                                   f"{dset_id}_downstream_{sample_num}samples_tuned_mlp_head_from_supervised_pretrain_ft-trans_seed{seed}.toml"),
                      "w") as fh:
                toml.dump(toml_dict, fh)
            ...
            with open(os.path.join(out_dir,
                                   f"{dset_id}_downstream_{sample_num}samples_linear_head_tuned_full_from_supervised_pretrain_ft-trans_seed{seed}.toml"),
                      "w") as fh:
                toml.dump(toml_dict, fh)
            ...
            toml_dict["transfer"]["use_mlp_head"] = True
            with open(os.path.join(out_dir,
                                   f"{dset_id}_downstream_{sample_num}samples_mlp_head_tuned_full_from_supervised_pretrain_ft-trans_seed{seed}.toml"),
                      "w") as fh:
                toml.dump(toml_dict, fh)
            ...
            launch_str = launch_str + f"python -W ignore bin/ft_transformer_mlm.py output/avi/{dset_id}_downstream_{sample_num}samples_fromscratch_ft-trans_seed{seed}.toml -f \n" \
                                      f"python -W ignore bin/ft_transformer_mlm.py output/avi/{dset_id}_downstream_{sample_num}samples_random_extractor_ft-trans_seed{seed}.toml -f \n" \
                                      f"python -W ignore bin/ft_transformer_mlm.py output/avi/{dset_id}_downstream_{sample_num}samples_tuned_mlp_head_from_supervised_pretrain_ft-trans_seed{seed}.toml -f \n" \
                                      f"python -W ignore bin/ft_transformer_mlm.py output/avi/{dset_id}_downstream_{sample_num}samples_linear_head_tuned_full_from_supervised_pretrain_ft-trans_seed{seed}.toml -f \n" \
                                      f"python -W ignore bin/ft_transformer_mlm.py output/avi/{dset_id}_downstream_{sample_num}samples_mlp_head_tuned_full_from_supervised_pretrain_ft-trans_seed{seed}.toml -f \n" \
                                      f"python -W ignore bin/ft_transformer_mlm.py output/avi/{dset_id}_downstream_{sample_num}samples_tuned_mlp_head_from_mlm_pretrain_ft-trans_seed{seed}.toml -f \n" \
                                      f"python -W ignore bin/ft_transformer_mlm.py output/avi/{dset_id}_downstream_{sample_num}samples_linear_head_tuned_full_from_mlm_pretrain_ft-trans_seed{seed}.toml -f \n" \
                                      f"python -W ignore bin/ft_transformer_mlm.py output/avi/{dset_id}_downstream_{sample_num}samples_mlp_head_tuned_full_from_mlm_pretrain_ft-trans_seed{seed}.toml -f \n"

        with open(f"../RTDL/launch/{dset_id}_exp_seed{seed}.sh", "w") as fh:
            fh.write(launch_str)