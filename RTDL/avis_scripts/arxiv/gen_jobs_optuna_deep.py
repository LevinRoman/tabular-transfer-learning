...
dset_ids = ['california_housing', 41169, 'jannis', 1596, 'year', 'aloi', 41166, 554, 541, 40975, 1483, 42726, 42728, 40687, 188, 43172]
...
for model in models:
    for dset_id in dset_ids:
        ...
        base_config_data, base_config_model, optimization_space_model, base_config_training, optimization_space_training, \
           base_config_transfer, optimization_options, optimization_sampler = get_model_config(model, dset_is_big)
        ...
        launch_str = f"python -W ignore bin/tune.py output/optuning_configs/deep/optuning_{dset_id}_{model}.toml -f \n"
        launch[dset_id] += launch_str

        os.makedirs('launch/optuna_jobs/deep', exist_ok=True)
        with open(f"launch/optuna_jobs/deep/optuning_{dset_id}_{model}.sh", "w") as fh:
            fh.write(launch_str)