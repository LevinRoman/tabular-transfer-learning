...
def data_prep_transfer_mimic(ds_id, seed, task, stage='pretrain', pretrain_proportion=0, downstream_samples_per_class = 2, column_mode=None, pretrain_subsample = False, input_seed=0):
    ...
    # mic = {0: 'avg_50931', 1: 'avg_220179', 2: 'age', 3: 'age', 4: 'first_226512', 5: 'min_51221', 6: 'age', 7: 'avg_220179',
    #  8: 'first_226512', 9: 'max_220045', 10: 'min_51265', 11: 'age'}
    to_drop = 'age' # mic[pretrain_proportion]
    to_drop_index = numerical_columns.index(to_drop)
    ...
    if 'min' in to_drop:
        the_col_name = to_drop.split('_')[1]
        avg_col = 'avg_' + the_col_name
        max_col = 'max_' + the_col_name

        #if column_mode == 'remove_column' or column_mode == 'predict_missing_column' or column_mode == 'train_to_predict_missing_column' or column_mode =='train_with_imputed_column':
        X_train = X_train.drop(columns=[avg_col, max_col])
        X_val = X_val.drop(columns=[avg_col, max_col])
        X_test = X_test.drop(columns=[avg_col, max_col])

        numerical_columns.remove(avg_col)
        numerical_columns.remove(max_col)
    ...
    if 'max' in to_drop:
        ...
        #if column_mode == 'remove_column' or column_mode == 'predict_missing_column' or column_mode == 'train_to_predict_missing_column' or column_mode =='train_with_imputed_column':
        X_train = X_train.drop(columns=[min_col, avg_col])
        X_val = X_val.drop(columns=[min_col, avg_col])
        X_test = X_test.drop(columns=[min_col, avg_col])
    ...
    if column_mode == 'train_to_predict_missing_column':
        ...
        print(f"The number of Nas is {Na_indexes.sum()}")
