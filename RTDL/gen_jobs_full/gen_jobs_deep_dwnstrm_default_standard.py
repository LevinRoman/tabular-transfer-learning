...
out_dir = f"output_mimic_full/deep_dwnstrm_default_standard"
...
for model in models:
    for downstream_target in downstream_targets:
        for dset_id in dset_ids:
            ...
            optuna_config_path = optuna_config_path = f"output/optuning_configs/deep/optuning_{dset_id}_{model}_{downstream_target}/best.toml"
            ...
            for seed in range(seeds):
                ...
                os.makedirs('launch_mimic_full/deep_dwnstrm_default_standard', exist_ok=True)
                with open(f"launch_mimic_full/deep_dwnstrm_default_standard/{dset_id}{downstream_target}_{model}_exp_seed{seed}.sh", "w") as fh:
                    fh.write(launch_str)
                global_launch_str += f"python3 dispatch_single_node.py launch_mimic_full/deep_dwnstrm_default_standard/{dset_id}{downstream_target}_{model}_exp_seed{seed}.sh --name {dset_id}{downstream_target}_{model}_exp_seed{seed} --time {time} \n"

os.makedirs('launch_mimic_full/global_launch', exist_ok=True)
with open(f"launch_mimic_full/global_launch/deep_dwnstrm_default_standard.sh", "w") as fh:
