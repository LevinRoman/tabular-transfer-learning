import toml
import os
#The results of this script would be identical to the results in output/optuning_configs/deep and output_mimic_dev/deep optuna
#This script is here for rerunning if needed
...
out_dir = f"output_mimic_full/deep_optuna"
...
for model in models:
    for downstream_target in downstream_targets:
        for dset_id in dset_ids:
            ...
            base_config_transfer['pretrain_subsample'] = False
            ...
            launch_str = f"python -W ignore bin/tune.py {out_dir}/optuning_{dset_id}_{model}_{downstream_target}.toml -f \n"

            os.makedirs('launch_mimic_full/deep_optuna', exist_ok=True)
            with open(f"launch_mimic_full/deep_optuna/optuning_{dset_id}_{model}_{downstream_target}.sh", "w") as fh:
                fh.write(launch_str)
            global_launch_str += f"python3 dispatch_single_node.py launch_mimic_full/deep_optuna/optuning_{dset_id}_{model}_{downstream_target}.sh --name optuning_{dset_id}_{model}_{downstream_target} --time {time} \n"

os.makedirs('launch_mimic_full/global_launch', exist_ok=True)
with open(f"launch_mimic_full/global_launch/deep_optuna.sh", "w") as fh:
