...
n_estimators = [1000]
time = 10
out_dir = f"output_mimic_full/catboost_upstream_subsample_tuning"
    ...
    for downstream_target in downstream_targets:
        for sample_num in sample_nums:
            for dset_id in dset_ids:
                ...
                os.makedirs('launch_mimic_full/catboost_upstream_subsample_tuning', exist_ok=True)
                with open(f"launch_mimic_full/catboost_upstream_subsample_tuning/optuning_{dset_id}{downstream_target}_num_samples{sample_num}_{model}.sh", "w") as fh:
                    fh.write(launch_str)
                global_launch_str += f"python3 dispatch_single_node.py launch_mimic_full/catboost_upstream_subsample_tuning/optuning_{dset_id}{downstream_target}_num_samples{sample_num}_{model}.sh --name optuning_{dset_id}{downstream_target}_num_samples{sample_num}_{model} --time {time} \n"

os.makedirs('launch_mimic_full/global_launch', exist_ok=True)
with open(f"launch_mimic_full/global_launch/catboost_upstream_subsample_tuning.sh", "w") as fh:
