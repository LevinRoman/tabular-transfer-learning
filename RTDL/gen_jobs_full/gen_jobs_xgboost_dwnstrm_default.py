...
n_estimators = [100]
out_dir = f"output_mimic_full/xgboost_dwnstrm_default"
    ...
    for downstream_target in downstream_targets:
        for dset_id in dset_ids:
            ...
            for seed in range(seeds):
                ...
                os.makedirs('launch_mimic_full/xgboost_dwnstrm_default', exist_ok=True)
                with open(f"launch_mimic_full/xgboost_dwnstrm_default/{dset_id}{downstream_target}_{model}_exp_seed{seed}.sh", "w") as fh:
                    fh.write(launch_str)
                global_launch_str += f"python3 dispatch_single_node.py launch_mimic_full/xgboost_dwnstrm_default/{dset_id}{downstream_target}_{model}_exp_seed{seed}.sh --name {dset_id}{downstream_target}_{model}_exp_seed{seed} --time {time} \n"

os.makedirs('launch_mimic_full/global_launch', exist_ok=True)
with open(f"launch_mimic_full/global_launch/xgboost_dwnstrm_default.sh", "w") as fh:
